...
export default function MobileDrawer({ open, onClose, onBgClick }: MobileDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-40 md:hidden">
      ...
      <div className="absolute inset-y-0 left-0 flex w-64 flex-col bg-white shadow-2xl dark:bg-gray-900">
        ...
        <nav className="flex-1 space-y-0.5 px-3">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              onClick={onClose}
              data-testid={item.path === '/calendar' ? 'nav-calendar' : undefined}
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-lg px-3 py-2.5 text-[14px] transition-all active:bg-gray-100 dark:active:bg-gray-800/60 ${
                  isActive
                    ? 'bg-gray-200/80 text-gray-900 font-medium dark:bg-gray-800/80 dark:text-gray-100'
                    : 'text-gray-500 active:text-gray-700 dark:text-gray-400 dark:active:text-gray-300'
                }`
              }
            >
        ...
        <div className="px-3 pb-2">
          <div className="mb-2 flex items-center justify-between px-3">
            ...
            <button onClick={() => setShowAdd(!showAdd)} className="rounded p-0.5 text-gray-400 transition-colors active:text-gray-600 dark:active:text-gray-300">
          ...
          {showAdd && (
            <div className="mb-2 flex gap-1 px-3">
              <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)}
                ...
                placeholder="项目名称"
                className="flex-1 rounded border border-gray-300 bg-white px-2 py-1.5 text-[12px] dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100" />
              <button onClick={handleAdd} className="rounded bg-blue-600 px-2 py-1.5 text-[11px] text-white">+</button>
            </div>
          )}

          <div className="space-y-0.5">
            {projects.map((project) => (
              <div key={project.id} className="group flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-[14px] text-gray-500 transition-colors active:bg-gray-100 active:text-gray-700 dark:text-gray-400 dark:active:bg-gray-800/40 dark:active:text-gray-300">
                <span className={`h-2.5 w-2.5 rounded-full ${colorMap[project.color] || 'bg-gray-500'}`}></span>
                <span className="flex-1">{project.name}</span>
                <button onClick={() => setDeleteProjectTarget({ id: project.id, name: project.name })}
                  className="rounded p-0.5 text-gray-400 opacity-0 transition-opacity active:text-red-500 group-hover:opacity-100">
        ...
        <div className="border-t border-gray-200 px-4 py-3 dark:border-gray-800/60 space-y-2">
          {onBgClick && (
            <button onClick={() => { onClose(); onBgClick(); }}
